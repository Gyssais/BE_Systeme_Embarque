         ...
         
{
    ...
    while(bufIndex < size)
    {
        XBee_UART_PutChar(pData[bufIndex]);
        bufIndex++;
    }
         ...
         
{
    ...
    *  If at least one byte is received within the time out interval, wait for more data */
    for (iCntr = 0u; iCntr < ((uint16)10u * timeOut); iCntr++)
    {
        ...
        *  XBee_UART_BL_CHK_DELAY_MS and check again until the timeOut specified.
        */
        else
        {
            CyDelay(XBee_UART_BL_CHK_DELAY_MS);
        }
    }
    ...
    while(XBee_UART_GetRxBufferSize() > 0u)
    {
        ...
        *count  =(*count) + tempCount;
        ...
        if(*count < size)
        {
            ...
            *  If not wait for additional XBee_UART_WAIT_EOP_DELAY ms.
            */
            if(pData[dataIndexCntr - 1u] != XBee_UART_PACKET_EOP)
            {
                CyDelay(XBee_UART_WAIT_EOP_DELAY);
            }
        }
